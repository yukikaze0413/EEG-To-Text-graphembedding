...
def empty_word_fields(sent_obj):
    """Keep sentence-level EEG samples even when ZuCo lacks word-level data."""
    sent_obj['word'] = []
    ...
    for sent in matdata:
        ...
        if not isinstance(word_data, float):
            ...
            for word in word_data:
                ...
                if has_fixations(word.nFixations):
            ...
            print(f'missing word-level data: subj:{subject_name} content:{sent.content}, keep sentence-level sample')
            sent_obj = {'content': sent.content}
            ...
            dataset_dict[subject_name].append(empty_word_fields(sent_obj))
            continue
